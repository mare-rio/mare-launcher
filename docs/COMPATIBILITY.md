...
| Python installer and browser checks on Linux, Windows and macOS | GitHub-hosted CI passed on all three systems. Tests cover failure/recovery, wrong-device protection, pairing-code handling, checksums, archive safety and bundle exclusions. | End-to-end Android installation was tested from Linux; Windows/macOS terminal wrappers and actual TV wireless pairing still need hardware testing. |
