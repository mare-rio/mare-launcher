...
5. **Branch protection/rulesets:** `main` requires a PR and native code-owner review, with zero blanket approvals and no bypass actors. A PR authored by a code owner satisfies ownership; other PRs need approval from a listed member. Stale approvals are dismissed when code changes, and review conversations must be resolved. Requiring a separate latest-push approval would also block a sole member’s own PR, so it is disabled for this authorship-or-review policy. A separate ruleset requires Linux, Windows, macOS and Android CI and protects branch history, also with no bypass actors. The initial branch-creation exception does not apply to changes to the existing `main`.
6. **Review ownership:** `.github/CODEOWNERS` assigns every file, including itself and all workflows, directly to the organisation’s current member, `@danielcavalli`. GitHub was live-tested: an individually named code owner can author a PR with zero separate approvals, whereas a team-owned file still waits for review on a team member’s own PR. Keep the explicitly named owners aligned with Maré Rio membership and write access; outside collaborators and apps must not be listed. GitHub evaluates the base branch’s CODEOWNERS, so an unapproved PR cannot remove its own approval requirement. Adding members does not automatically add them as code owners. [GitHub code-owner rules](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners).
